...
use macerator::{Scalar, Simd, VAdd, VDiv, VMul, VSub, Vector};
use paste::paste;
    ...
    E: Scalar,
{
    ...
    unsafe fn vload_unaligned<S: Simd>(ptr: *const Self) -> Vector<S, Self> {
        unsafe {
            let vec_e: Vector<S, E> = E::vload_unaligned(ptr as *const E);
            std::ptr::read(&vec_e as *const Vector<S, E> as *const Vector<S, Self>)
        }
    }
    ...
    unsafe fn vload_low<S: Simd>(ptr: *const Self) -> Vector<S, Self> {
        unsafe {
            let vec_e: Vector<S, E> = E::vload_low(ptr as *const E);
            std::ptr::read(&vec_e as *const Vector<S, E> as *const Vector<S, Self>)
        }
    }
